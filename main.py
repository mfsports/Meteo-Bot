    ...
    if "text" in data["message"]:
        ...
        if message_text == "/start":
            send_telegram_message(
                chat_id,
                "👋 Salut cycliste !\n\nTu peux :\n\n📍 *Partager ta localisation* pour obtenir la météo précise\n🏙️ *Ou simplement taper le nom d'une ville* pour la consulter\n\nJe t'afficherai :\n- La météo actuelle\n- Les 6 prochaines heures\n- Le sens du vent\n- Et vers où partir pour rentrer vent dans le dos 💨",
                reply_markup=get_main_keyboard()
            )
        elif message_text == "📍 Localisation":
            send_telegram_message(chat_id, "📡 Partage ta position via le bouton ci-dessous.")
        else:
            user_state[chat_id] = "awaiting_city"
            send_telegram_message(chat_id, "🧭 Tu peux taper le *nom d'une ville* pour voir la météo 🏙️")
